...
async function main() {
  ...
  try {
    ...
    // Copy test sources if available (generated by download.sh)
    const projectRoot = path.resolve(__dirname, '..');
    const testSourcesSrc = path.join(projectRoot, 'pyspec', version, 'test_sources.json');
    if (fs.existsSync(testSourcesSrc)) {
      const testSourcesDst = path.join(versionDir, 'test_sources.json');
      fs.copyFileSync(testSourcesSrc, testSourcesDst);
      console.log(`Copied test_sources.json to ${testSourcesDst}`);
    }
